...
plot_region <- function(region) {
  ...
  williams_data <- williams_data[, c(ncol(williams_data) - 1, seq(2, 32, by = 2))] # columns of interest
  x <- as.data.frame(lapply(williams_data[, 2:ncol(williams_data)], as.numeric)) # make columns of interest numeric
  ...
  submidatlantic <- dat[dat$State %in% c("Virginia", "Deleware", "DistrictofColumbia", "NewJersey", "WestVirginia", "Pennsylvania"), ]
  ...
  } else { stop("Input any valid state or Notable. Example: plot_region('MountainWest')") }
}
